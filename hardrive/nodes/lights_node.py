...
from hardrive.hardrive_config import config, HELMET_LIGHTS
import rclpy
...
from hardrive.hardrive_config import config, JSON_HELMET
...
class LightsNode(Node):
    def __init__(self):
        ...
        hardrive_json_manager = HardDriveJsonManager(config)
        ...
        self.lights = Lights(config=config, start=config[LIGHTS_START_LED], end=config[LIGHTS_END_LED],
            json_manager=hardrive_json_manager, global_strip=pixels, light_type=HELMET_LIGHTS, json_light=JSON_HELMET)
